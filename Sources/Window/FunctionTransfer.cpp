...
namespace Window
{
    ...
    FunctionTransfer::FunctionTransfer(QObject *parent) : QObject(parent)
    {
        ...
        connect(this, SIGNAL(comming(std::function<void()>)), this, SLOT(slotExec(std::function<void()>)),
                Qt::BlockingQueuedConnection);
        connect(this, SIGNAL(comming_noBlock(std::function<void()>)), this, SLOT(slotExec(std::function<void()>)),
                Qt::QueuedConnection);
    }
